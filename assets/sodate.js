/* =========================================================
   かもしてぽん！そだてモード（プロトタイプ）
   3つそろえると消えるのではなく「つぎの素材にそだつ」。
   🌾おこめ → 🍚むしまい → 🫧もろみ → 🍶仕込み樽（→発酵・収穫は本編と同じ）
...
(() => {
  ...
  // 素材タイル（grow=そろえたときに育つ先。♨️🧊は特殊パネル）
  const TYPES = [
    { id: "kome",    emoji: "🌾", name: "おこめ",   grow: "mushi" },
    { id: "mushi",   emoji: "🍚", name: "むしまい", grow: "moromi" },
    { id: "moromi",  emoji: "🫧", name: "もろみ",   grow: "barrel" },
  ...
  // 初期ボードの配合（ピラミッド配置：🫧2つが最初のニンジン）
  const INITIAL_MIX = { kome: 16, mushi: 6, moromi: 2, atsu: 4, hiyashi: 4, mizu: 4 };
  // 補充の出現率（%）。🫧は補充されない＝盤上でそだてるしかない
  ...
  const BEST_KEY = "kamoshitepon_sodate_best_v1";
  ...
  // 補充タイルの抽選（重みつき。育てる経済を守るため🫧は出ない）
  const randType = () => {
    let roll = Math.random() * 100;
    for (const [id, w] of REFILL_WEIGHTS) {
      roll -= w;
      if (roll < 0) return TYPE_INDEX[id];
    }
    return TYPE_INDEX.kome;
  };
  ...
  // ---------- 盤面初期化（配合表どおりにタイルを用意してシャッフル） ----------
  function initBoard() {
    const bag = [];
    for (const [id, n] of Object.entries(INITIAL_MIX)) {
      for (let i = 0; i < n; i++) bag.push(TYPE_INDEX[id]);
    }
    ...
    for (let r = 0; r < SIZE; r++) {
      ...
      for (let c = 0; c < SIZE; c++) {
        board[r].push(makeTile(bag[k++]));
      }
    }
    // 初期マッチは別のマスと入れかえてほどく（配合をくずさない）
    let guard = 0;
    while (findMatches().length > 0 && guard++ < 300) {
      const { r, c } = findMatches()[0].cells[0];
  ...
  async function resolveBoard(originCell) {
    ...
    while (groups.length > 0) {
      ...
      for (const group of groups) {
        ...
        const first = group.cells[0];
        const typeDef = TYPES[board[first.r][first.c].type];

        // スポーン位置の優先順：入れかえ先 → グループのほかのマス
        const spots = [...group.cells];
        if (originCell) {
          spots.sort((a, b) => {
            const ao = a.r === originCell.r && a.c === originCell.c ? 0 : 1;
            const bo = b.r === originCell.r && b.c === originCell.c ? 0 : 1;
            return ao - bo;
          });
        }

        if (typeDef.grow === "barrel") {
          // 🫧もろみ×3 → 仕込み樽（樽置き場の空きが必要）
          if (barrelsNow + barrelSpawns.length < BARREL_CAP) {
            ...
            barrelSpawns.push({ ...spots[0], mult, fromEmoji: typeDef.emoji });
          } else if (!yardFullToasted) {
            yardFullToasted = true;
            toast("樽置き場がいっぱい！そろえたぶんは得点に🌾", 2);
          }
        } else if (typeDef.grow) {
          // 育成タイル：そろえると次の素材にそだつ（4つ消しで2つ、5つで3つ）
          const count = group.len >= 5 ? 3 : group.len >= 4 ? 2 : 1;
          for (let i = 0; i < count && i < spots.length; i++) {
            growthSpawns.push({
              ...spots[i],
              typeIndex: TYPE_INDEX[typeDef.grow],
              fromEmoji: typeDef.emoji,
            });
          }
        }
        // 道具タイル（♨️🧊💧）は効果を発動してそのまま消える
      ...
      for (const g of growthSpawns) {
        board[g.r][g.c] = makeTile(g.typeIndex);
      }
      let bestMult = 0;
      ...
      // そだった瞬間のお披露目：落下前にいったん描画して、ぽよん＆変身チップ
      if (growthSpawns.length || barrelSpawns.length) {
        const growClasses = {};
        for (const g of growthSpawns) growClasses[`${g.r},${g.c}`] = "grow";
        for (const s of barrelSpawns) growClasses[`${s.r},${s.c}`] = "grow";
        render(growClasses);
        for (const g of growthSpawns) {
          const def = TYPES[g.typeIndex];
          spawnChip(g.r, g.c, `${g.fromEmoji}→${def.emoji} ${def.name}！`);
        }
        for (const s of barrelSpawns) {
          spawnChip(s.r, s.c, `${s.fromEmoji}→🍶 仕込み樽！`);
        }
        await sleep(500);
      }
